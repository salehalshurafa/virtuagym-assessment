...
import WeeklyPlanDialog from '@/components/WeeklyPlanDialog.vue'

const props = defineProps<{
  weeklyPlans: WeeklySplitTemplate[]
}>()
...
const visibleWeeks = computed(() => {
  ...
  if (q) {
    return props.weeklyPlans.filter((w) => w.label.toLowerCase().includes(q))
  }
  return props.weeklyPlans.slice(0, SUMMARY_LIMIT)
})
</script>
...
<template>
  <section class="flex flex-col gap-4 border border-[#3a3a37] rounded-md p-4">
    <header class="flex items-center justify-between">
      <h2 class="text-2xl font-bold text-white">Weekly Split Templates</h2>
    ...
    <div class="relative">
      <Input
        ...
        placeholder="Search weekly templates..."
        aria-label="Search weekly templates"
      />
      <button
        v-if="searchQuery"
        type="button"
    ...
    <ul
      v-if="visibleWeeks.length"
      class="flex flex-col gap-2 max-h-[11rem] overflow-y-auto pr-1 scrollbar-thin"
    >
      <li v-for="week in visibleWeeks" :key="week.id">
        <WeeklyPlanDialog :weekly-plan="week" />
      </li>
    </ul>
    <p v-else class="text-sm text-[#a0a09a] text-center">No weekly templates match your search</p>
  </section>
</template>
